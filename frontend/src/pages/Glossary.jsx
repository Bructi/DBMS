...
import { useState, useEffect } from "react";
import { Search, BookOpen, ArrowLeft, Tag, Globe, Loader2, AlertCircle, ChevronDown, ChevronUp } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
...
export default function Glossary() {
  ...
  // 1. Local Database
  const localDictionary = [
    { term: "RSI (Relative Strength Index)", category: "Technical", definition: "A momentum indicator measuring the speed and magnitude of recent price changes. Over 70 is 'Overbought', under 30 is 'Oversold'." },
    { term: "SMA (Simple Moving Average)", category: "Technical", definition: "The average price of a stock over a specific number of days. It smooths out daily price fluctuations to show the overall trend." },
    { term: "P/E Ratio (Price-to-Earnings)", category: "Fundamental", definition: "Measures a company's current share price relative to its per-share earnings." },
    { term: "LTP (Last Traded Price)", category: "General", definition: "The exact price at which the most recent transaction of a stock occurred in the live market." },
    { term: "NAV (Net Asset Value)", category: "General", definition: "The per-share value of a mutual fund or ETF." },
    { term: "Bull Market", category: "General", definition: "A financial market condition where prices are rising persistently." },
    { term: "Bear Market", category: "General", definition: "A financial market condition in which prices are falling, typically by 20% or more." }
  ];

  const categories = ["All", "General", "Technical", "Fundamental", "Web Results"];
  ...
  // SMART AUTO-SUGGEST ENGINE (Triggers as you type)
  useEffect(() => {
    ...
    const fetchSuggestions = async () => {
      try {
        // 1. Get matches from our local database
        const localMatches = localDictionary
          .filter(item => item.term.toLowerCase().includes(searchTerm.toLowerCase()))
          .map(item => item.term);

        // 2. Fetch live suggestions from Wikipedia OpenSearch API
        const res = await fetch(`https://en.wikipedia.org/w/api.php?action=opensearch&search=${encodeURIComponent(searchTerm)}&limit=5&format=json&origin=*`);
        const data = await res.json();
        const webMatches = data && data[1] ? data[1] : [];

        // 3. Combine them, remove duplicates, and limit to top 6 results
        const combined = [...new Set([...localMatches, ...webMatches])].slice(0, 6);
    ...
    // Debounce to prevent spamming the API while typing fast
  ...
  const executeWebSearch = async (termToSearch) => {
    ...
    try {
      const searchQuery = encodeURIComponent(termToSearch + " finance");
      const response = await fetch(`https://en.wikipedia.org/w/api.php?action=query&format=json&origin=*&generator=search&gsrsearch=${searchQuery}&gsrlimit=1&prop=extracts&exintro=true&explaintext=true`);
      const data = await response.json();
      
      if (!data.query || !data.query.pages) {
        setWebError(`We couldn't find a standard financial definition for "${termToSearch}".`);
      } else {
        const pages = data.query.pages;
        const pageId = Object.keys(pages)[0];
        const page = pages[pageId];
      ...
      setWebError("Make sure you are connected to the internet to fetch live definitions.");
    } finally {
      setIsSearchingWeb(false);
    }
  };
  ...
  const handleSuggestionClick = (suggestion) => {
    ...
    // If they clicked a local dictionary term, just show the local card immediately!
    const isLocal = localDictionary.some(item => item.term === suggestion);
    if (isLocal) {
      setActiveCategory("All");
      setWebResult(null);
      setWebError("");
    } else {
      // If it's a web suggestion, execute the web search
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      <div className="max-w-5xl mx-auto">
        ...
        {/* Header Section */}
        <div className="flex items-center gap-4 mb-8">
          <button onClick={() => navigate(-1)} className="p-2 bg-white border border-gray-200 rounded-lg hover:bg-gray-100 text-gray-600 transition">
            <ArrowLeft size={20} />
          </button>
          <div>
            <h1 className="text-3xl font-black text-gray-900 flex items-center gap-2">
              <Globe className="text-blue-600" size={28}/> Global Market Dictionary
            </h1>
            <p className="text-gray-500 font-medium">Search our local database or fetch live definitions from the web.</p>
          </div>
        </div>
          ...
          <form onSubmit={handleFormSubmit} className="flex gap-2">
            ...
            <div className="relative flex-1">
              ...
              <input 
                ...
                onBlur={() => setShowSuggestions(false)} // Closes dropdown if clicked outside
                className="w-full pl-12 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-blue-500 outline-none font-medium text-gray-700 transition"
              />
                  ...
                  >
                    {suggestions.map((suggestion, idx) => (
                      <div 
                        ...
                        className="px-5 py-3 hover:bg-gray-50 cursor-pointer flex items-center gap-3 text-gray-700 font-bold border-b border-gray-100 last:border-0 transition"
                      >
          ...
          <div className="flex flex-wrap gap-2">
            {categories.map(category => (
              <button 
                ...
                className={`px-4 py-2 rounded-lg text-sm font-bold transition flex items-center gap-1
                  ${activeCategory === category 
                    ? category === 'Web Results' ? 'bg-gray-900 text-white' : 'bg-blue-600 text-white shadow-md' 
                    : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}
              >
                {category === "Web Results" ? <Globe size={14}/> : category !== "All" && <Tag size={14}/>} 
                {category}
              </button>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <AnimatePresence mode="popLayout">
            ...
            {/* Show Local Terms */}
            {activeCategory !== "Web Results" && filteredLocalTerms.map((item, index) => (
              <motion.div 
                ...
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`text-xs font-bold px-2 py-1 rounded uppercase tracking-wider
                    ${item.category === 'Technical' ? 'bg-purple-100 text-purple-700' : 
                      item.category === 'Fundamental' ? 'bg-emerald-100 text-emerald-700' : 
                      'bg-blue-100 text-blue-700'}`}
                  >
                    {item.category}
                  </span>
            ...
            {/* Show Web Search State */}
            {activeCategory === "Web Results" && isSearchingWeb && (
              <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="col-span-full flex flex-col items-center justify-center py-16 text-gray-500">
                <Loader2 className="animate-spin text-blue-600 mb-4" size={32} />
                <p className="font-bold">Scouring the web for "{searchTerm}"...</p>
              </motion.div>
            )}

            {/* Show Web Error */}
            {activeCategory === "Web Results" && webError && !isSearchingWeb && (
              <motion.div layout initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="col-span-full bg-red-50 border border-red-100 text-red-600 p-6 rounded-2xl flex items-center gap-3">
                <AlertCircle size={24} />
                <div>
                  <h4 className="font-bold">No exact match found</h4>
                  <p className="text-sm">{webError} Try a more common financial term.</p>
                </div>
              </motion.div>
            )}

            {/* Show Web Result */}
            {activeCategory === "Web Results" && webResult && !isSearchingWeb && (
              <motion.div 
              ...
              >
                ...
                {webResult.definition.length > 350 && (
                  <button 
                    ...
                    className="mt-4 text-blue-400 hover:text-blue-300 font-bold text-sm flex items-center gap-1 transition-colors"
                  >
                    {isExpanded ? <><ChevronUp size={16}/> Read Less</> : <><ChevronDown size={16}/> Read More</>}
                  </button>
                )}

                <div className="mt-6 pt-4 border-t border-gray-700 text-xs text-gray-500 font-bold flex items-center gap-2">
                  <BookOpen size={14}/> Sourced dynamically from Wikipedia API
                </div>
              </motion.div>
            )}

            {/* Empty Local State */}
            {activeCategory !== "Web Results" && filteredLocalTerms.length === 0 && searchTerm && !showSuggestions && (
               <motion.div layout className="col-span-full text-center py-12 text-gray-500">
                 <Globe size={32} className="mx-auto mb-3 text-gray-300" />
                 <p className="font-bold text-lg text-gray-700">"{searchTerm}" isn't in our local quick-cache.</p>
                 <p className="text-sm mt-1">Press <b>Enter</b> or click <b>Deep Web Search</b> to find it online!</p>
               </motion.div>
            )}
